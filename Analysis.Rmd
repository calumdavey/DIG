...
summary_cells <- function(v) {
    ...
    if (length(unique(v)) < 4) {
        ...
        paste0(format(mean(v, na.rm = T), big.mark = ",", digits = 0, scientific = F), " (",
               format(sd(v, na.rm = T), big.mark = ",", digits = 0, scientific = F), ")")
    }
}
...
* Is there a "this person is disabled at baseline" variable that we can use to restrict the data?
...
table1[2:nrow(table1), ] |>
    knitr::kable(col.names = c("Control", "DIG Arm")) |>
