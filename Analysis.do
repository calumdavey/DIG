...
replace resp_lino=lino if follow==0 // Order was randomized after baseline, so am recovering baseline order
keep if s215==resp_lino
...
* Index person PWD: Washington group extended set 
...
*gen index_diff_remember=(s2_18==3 | s2_18==4)
...
* Primary outcome
	* Per-capita household consumption (dollar amount)
*		--> scored as in the baseline 
...
egen food_expenses =rowtotal(buy_s42e_*)
drop buy_s42e_*
replace food_expenses=food_expenses*12
label var food_expenses "Annual food expenditure"

* Non-food recurrent and infrequent expenses
egen recurrent_expenses =rowtotal(s43_b_*)
replace recurrent_expenses=recurrent_expenses*12
label var recurrent_expenses "Annual non-food recurrent expenditure"

egen infrequent_expenses =rowtotal(s44_b_*)
...
gen percapitaexpenditure=hh_expenditure/tot_hhmem_num
...
* Animal and animal products sales
...
* Index person participation in livelihood activities
* ==================================

*2.	Participation of the project participant (with disability) in livelihood activities // Index PWD there could be multiple PWD in the household.
*		--> look at the individual level (PWD)
use "Data/DIG Household Panel Wide.dta", clear
...
* ==================================
*3.	Participation of the project participant (with disability) in social activities
*		--> look at the questions we have and make a score from the various questions 
* Training
...
* Analysis
* =============================================================================================== *
save "Data/DIG Household Panel Wide Clean.dta", clear
keep if endline==1

local vars_sociodem femalehhh tot_hhmem_num resp_s24 married_resp resp_s212 cement_wall improved_latrine one_phone two_phone radio s312 s313 one_meal two_meal three_meal hhpwd

iebaltab `vars_sociodem' if round==0 & treat==0, grpvar(res_gender) save("${DIREC_OUTPUT}/tables/socio-economic.xlsx") control(2) total replace rowvarlabels vce(robust)
...
* Baseline levels of demographic and outcome variables
